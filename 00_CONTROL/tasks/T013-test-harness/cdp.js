...
async function connectToNewTab(cdpHttpBase) {
  ...
  // T013-specific: real network-layer offline emulation via CDP, distinct
  // from unplugging a physical cable but a genuine network-layer cutoff —
  // Network.emulateNetworkConditions with offline:true makes the renderer's
  // fetch/XHR/service-worker fetch events actually fail at the network
  // layer, which is the same mechanism DevTools' "Offline" checkbox uses.
  ...
  return { navigate, evaluate, getConsole, getRequestUrls, close, setOffline };
}
